...
exports.handler = async (event) => {
  ...
  const roleId = process.env.DISCORD_ROLE_ID || "const { parseCookies, cookie, createSessionCookie } = require("./_session");
...
exports.handler = async (event) => {
  ...
  const guildId = process.env.DISCORD_GUILD_ID;
  const botToken = process.env.DISCORD_BOT_TOKEN;
  const roleId = process.env.DISCORD_ROLE_ID || "1393247773067055174";
